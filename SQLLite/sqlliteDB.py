...
def DBSort():
    ...
    cur.execute('create table if not exists ' + TName +  ' (Val INTEGER, Val VARCHAR(41))')
    cur.execute('DELETE FROM ' + TName)
    Vector=(numpy.random.randint(-50, 50, 30000000)).tolist()
    for i in Vector:
        cur.execute('INSERT INTO   Sort2   VALUES (' + str(i) +', '+ Sha_hash(i) +')')
    con.commit()
...
def CreateAndFill():
    ...
    cur.execute('create table if not exists   STUDENTS1  (Firstname VARCHAR[60], Math INTEGER, Phisic INTEGER, Geometry INTEGER, Economic INTEGER, Informatic INTEGER)')
    ...
    cur.executemany('INSERT INTO STUDENTS1 VALUES (?,?,?,?,?,?)', purchases1)
...
if __name__ == "__main__":
    ...
    #cur.execute('create table if not exists   STUDENTS2  (Firstname VARCHAR[60], Math INTEGER, Phisic INTEGER, Geometry INTEGER, Economic INTEGER, Informatic INTEGER)')
    print(Sha_hash("jkldsflsdjkl;sdf"))
